...
def is_production_source(path: str) -> bool:
    ...
    return (
        ...
        or (normalized.startswith("web/") and normalized.endswith((".js", ".mjs")))
    )
...
def run_selection(selection: Selection) -> int:
    comfyui_root = Path.cwd().resolve()
    if not (comfyui_root / "folder_paths.py").is_file():
        raise RuntimeError("Run tests/run_tests.py from the configured ComfyUI root.")
